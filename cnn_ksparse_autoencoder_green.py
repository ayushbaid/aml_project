...
if __name__ == '__main__':
    ...
    loaded_data = loadmat(os.path.join(base_path, 'train_images/concat_g.mat'))

    input_data = np.expand_dims(loaded_data['concat_g'],3)
    loaded_data = None

    sess.run(train_step, feed_dict=
                {x:input_data, max_nonzero:NUM_NONZERO})

    print("Low Sparsity Green Optimization Finished!")
    saver.save(sess,"./cnn/trainedModelNorm_green_lowsparse.ckpt")

    w_np = {'U':sess.run(W)}
    # Storing w as mat file
    savemat('cnn/dict_green_low.mat', w_np)
    ...
    print("Medium Sparsity Green Optimization Finished!")
    saver.save(sess,"./cnn/trainedModelNorm_green_medsparse.ckpt")

    w_np = {'U':sess.run(W)}
    # Storing w as mat file
    savemat('cnn/dict_green_med.mat', w_np)
    ...
    print("High Sparsity Green Optimization Finished!")
    saver.save(sess,"./cnn/trainedModelNorm_green_highsparse.ckpt")

    w_np = {'U':sess.run(W)}
    # Storing w as mat file
    savemat('cnn/dict_green_high.mat', w_np)
