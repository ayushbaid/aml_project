...
if __name__ == '__main__':
    ...
    loaded_data = loadmat(os.path.join(base_path, 'train_images/concat_b.mat'))

    input_data = np.expand_dims(loaded_data['concat_b'],3)
    loaded_data = None

    sess.run(train_step, feed_dict=
                {x:input_data, max_nonzero:NUM_NONZERO})

    print("Low Sparsity Blue Optimization Finished!")
    saver.save(sess,"./cnn/trainedModelNorm_blue_lowsparse.ckpt")

    w_np = {'U':sess.run(W)}
    # Storing w as mat file
    savemat('cnn/dict_blue_low.mat', w_np)
    ...
    print("Medium Sparsity Vlue Optimization Finished!")
    saver.save(sess,"./cnn/trainedModelNorm_blue_medsparse.ckpt")

    w_np = {'U':sess.run(W)}
    # Storing w as mat file
    savemat('cnn/dict_blue_med.mat', w_np)
    ...
    print("High Sparsity Blue Optimization Finished!")
    saver.save(sess,"./cnn/trainedModelNorm_blue_highsparse.ckpt")

    w_np = {'U':sess.run(W)}
    # Storing w as mat file
    savemat('cnn/dict_blue_high.mat', w_np)
